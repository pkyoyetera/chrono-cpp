...
#ifndef ENRELATIVEDATEFORMAT_HPP
#define ENRELATIVEDATEFORMAT_HPP

#define PATTERN "(\\W|^)(this|next|last|past)\\s*( (?:one|two|three|four|five|six|seven|" \
				"eight|nine|ten|eleven|twelve)|[0-9]+|few|half(?:\\s*an?)?)?\\s*(seconds?|" \
				"min(?:ute)?s?|hour(?:s)?|day(?:s)?|week(?:s)?|month(?:s)?|year(?:s)?)(?=\\s*)" \
				"(?=\\W|$)"

#define MODIFIER_WORD_GROUP   2
#define MULTIPLIER_WORD_GROUP 3
#define RELATIVE_WORD_GROUP   4
...
public:
	ENRelativeDateFormatParser() : Parser(false, std::regex(PATTERN, std::regex::icase)) { }

	parse::ParsedResult extract(std::string, std::smatch match, posix_time::ptime& ref, long idx)
	override {
		...
		/*short modifier;
		std::regex_match(match.str(MODIFIER_WORD_GROUP),
						 std::regex("next", std::regex::icase)) ? modifier = 1 : modifier = -1;*/
		...
		if (utils::INTEGER_WORDS.find(num_label) != utils::INTEGER_WORDS.end()) {
			...
			try {
				...
				std::cerr << e.what() << ":at ENRelativeDateFormatParser" << std::endl;
			}
...
#undef MODIFIER_WORD_GROUP
#undef MULTIPLIER_WORD_GROUP
#undef RELATIVE_WORD_GROUP
